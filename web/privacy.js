...
(() => {
  ...
  const I18N = {
    en: {
      ...
      pp_meta_desc: "How the free score.seoryon.com tool, the /api/score backend, and the oryon-score pip package handle your data. Plain English, no boilerplate.",
      pp_back: "← Back to Score",
      pp_back_short: "Score",

      pp_h1: "Privacy Policy",
      pp_updated: "Last updated: 26 June 2026",
      pp_intro: "This policy covers the free scoring tool at score.seoryon.com, the /api/score backend that powers it, and the open-source oryon-score Python package. All three are made by SEOryon. Short version: there is no account, no tracking, no analytics — but the scorer does have a small server-side component, and we want to be specific about what it sees.",
      ...
      pp_s3_b4: "Outbound links to app.seoryon.com, seoryon.com, github.com, and pypi.org take you to separate properties with their own privacy terms.",

      pp_s4_title: "The oryon-score pip package",
      pp_s4_p: "The same scoring logic ships as an open-source Python package (pip install oryon-score). When you run it locally, the package fetches the URL you give it directly from your machine — nothing is sent to SEOryon, and we never see what you scored. The code is auditable on GitHub under MIT.",
    ...
    de: {
      ...
      pp_meta_desc: "Wie das kostenlose Tool auf score.seoryon.com, das Backend /api/score und das Pip-Paket oryon-score mit deinen Daten umgehen. In Klartext, ohne Floskeln.",
      pp_back: "← Zurück zum Score",
      pp_back_short: "Score",

      pp_h1: "Datenschutzerklärung",
      pp_updated: "Zuletzt aktualisiert: 26. Juni 2026",
      pp_intro: "Diese Erklärung gilt für das kostenlose Score-Tool auf score.seoryon.com, das Backend /api/score, das es antreibt, und das Open-Source-Python-Paket oryon-score. Alle drei stammen von SEOryon. Kurz gesagt: Es gibt kein Konto, kein Tracking, keine Analyse-Tools – aber der Scorer hat eine kleine serverseitige Komponente, und wir wollen genau sagen, was sie sieht.",
      ...
      pp_s3_b4: "Ausgehende Links zu app.seoryon.com, seoryon.com, github.com und pypi.org führen zu eigenständigen Angeboten mit eigenen Datenschutzbedingungen.",

      pp_s4_title: "Das Pip-Paket oryon-score",
      pp_s4_p: "Die gleiche Scoring-Logik gibt es auch als quelloffenes Python-Paket (pip install oryon-score). Wenn du es lokal ausführst, ruft das Paket die von dir übergebene URL direkt von deinem Rechner ab – an SEOryon wird nichts gesendet, und wir sehen nie, was du geprüft hast. Der Code ist auf GitHub unter MIT-Lizenz einsehbar.",
    ...
    fr: {
      ...
      pp_meta_desc: "Comment l'outil gratuit score.seoryon.com, le backend /api/score et le paquet pip oryon-score traitent vos données. Dit clairement, sans formules toutes faites.",
      pp_back: "← Retour au Score",
      pp_back_short: "Score",

      pp_h1: "Politique de confidentialité",
      pp_updated: "Dernière mise à jour : 26 juin 2026",
      pp_intro: "Cette politique couvre l'outil gratuit de scoring sur score.seoryon.com, le backend /api/score qui l'alimente et le paquet Python open source oryon-score. Les trois sont édités par SEOryon. En bref : pas de compte, pas de suivi, pas d'outils d'analyse — mais le scoreur comporte une petite partie côté serveur, et nous voulons dire précisément ce qu'elle voit.",
      ...
      pp_s3_b4: "Les liens sortants vers app.seoryon.com, seoryon.com, github.com et pypi.org mènent à des propriétés distinctes, qui ont leurs propres conditions de confidentialité.",

      pp_s4_title: "Le paquet pip oryon-score",
      pp_s4_p: "La même logique de scoring est aussi distribuée sous forme de paquet Python open source (pip install oryon-score). Quand vous l'exécutez en local, le paquet récupère l'URL que vous lui passez directement depuis votre machine — rien n'est envoyé à SEOryon, et nous ne voyons jamais ce que vous avez analysé. Le code est auditable sur GitHub, sous licence MIT.",
